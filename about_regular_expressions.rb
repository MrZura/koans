...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    # Никогда, если считать неудачным поиском тот, который выдает nil.
    # Попробовал сделать поиск по пустой строке, результат - "".
  end
  ...
  #
  # Why?
  # Greedy оператор возьмёт как можно больше символов, удовлетворяющих условию поиска,
  # например "abbcccddddeeeee"[/ab*/] вернет "abb".
  # Но Lazy оператор вернет минимально удовлетворяющий результат, например
  # "abbcccddddeeeee"[/ab*?/] вернет "a".
  # ------------------------------------------------------------------
  ...
  # Поиск с помощью набора символов ([...]) осуществляется только по одному символу исходной строки.
  # Т.е. каждый символ исходной строки сопоставляется набору символов из регулярного выражения.
  # Alternation же может осуществлять поиск, как по наборам символов ([..]|[..]|abc), так и по
  # определенной последовательности символов, позволяя находить более вариативные подстроки.
  #
  # ------------------------------------------------------------------
